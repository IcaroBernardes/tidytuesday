...
df <- tidytuesdayR::tt_load('2021-12-21')$starbucks

## Selects the biggest size of the drinks and only the relevant variables
df <- df %>% 
  dplyr::filter(product_name == "Cinnamon Dolce Latte") %>% 
  dplyr::select(product_name, milk, whip, calories, saturated_fat_g,
                trans_fat_g, sodium_mg, total_carbs_g, sugar_g, caffeine_mg)

## Converts characters to numeric
df <- df %>% 
  dplyr::mutate(trans_fat_g = as.numeric(trans_fat_g))
